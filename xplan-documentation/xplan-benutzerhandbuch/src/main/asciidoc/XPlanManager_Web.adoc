...
Im Abschnitt <<referenzierung-von-rasterdaten-im-xplangml>> wurde bereits darauf hingewiesen, dass in den Versionen 5.1 und 5.2 zwei Varianten zur Referenzierung von Rasterdaten möglich sind. Beim Editieren von Plänen wird nur die alte Version unterstützt. Unabhängig ob im importierten Plan die neue oder alte Variante verwendet wurde, wird in dem editierten Plan immer die Referenzierung über den Feature Type XP_Rasterdarstellung erfolgen.
