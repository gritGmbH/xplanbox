 ...
 * Rasterdaten sowie alle im __xplan.gml__ referenzierten Anhänge liegen ebenfalls im Basisverzeichnis der ZIP-Datei.

NOTE: *Wichtig*: Dateinamen dürfen keine Sonderzeichen wie z.B. '/', '\', ' ', '"' (doppeltes Anführungszeichen) oder auch Umlaute beinhalten.
Gross- und Kleinschreibung in den Referenzen sind zu beachten!**
...
In XPlanGML 5.1 ist diese Referenzierung als veraltet notiert. Stattdessen werden Rasterdateien über die von XP_Bereich abgeleiteten Feature Types referenziert (im folgendem Beispiel BP_Bereich):
