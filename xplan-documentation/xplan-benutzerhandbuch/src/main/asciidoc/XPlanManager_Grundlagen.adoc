 ...
 * Das XPlanGML-Dokument beinhaltet nur ein oder mehrere Elemente vom Typ `<xplan:XP_PLAN/>`. Beinhaltet eine Datei mehrere dieser Elemente, so dürfen keine nicht referenzierte Features im XPlanGML-Dokument vorkommen, wie z.B. ein freies Präsentationsobjekt ohne Bezug zu einem Bereich!
...
Beim Import von Planarchiven mit Rasterdaten werden Konfigurationsdateien für den XPlanWMS automatisch angelegt, die eine Visualisierung im XPlanWMS ermöglichen.
