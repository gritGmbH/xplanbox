...
Usage: XPlanManager <options>
 ...
 --createMetadata <planid> [<planid>..] [--managerconfiguration=<PFAD/ZU/VERZEICHNIS/MIT/MANAGERCONFIGURATION>]
     <planid> Die ID des Plans zu dem der Service-Metadatensatz generiert werden soll. Mehrere IDs koennen durch ein Leerzeichen getrennt angegeben werden. Wenn keien ID angegeben ist, werden f?r alle Plaene Metadatensaetze erstellt.
