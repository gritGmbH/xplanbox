...
der Konfigurationserzeugung für einen neuen Service genutzt werden
...
erfolgt mit dem Parameter `force`.

CAUTION: Bitte beachten Sie, dass dabei vorhandene Geometriefehler o.ä.
...
diese mit dem Parameter `crs` übergeben werden.
...
Alternativ zum normalen Import von Plänen bietet das XPlanManagerCLI die
...
vier verschiedene Optionen für das XPlanManagerCLI.

*Datenbank erzeugen:*

Mit dem XPlanManagerCLI kann die Datenhaltung erzeugt
...
Service genutzt werden soll (für weitere Details siehe _Konfiguration
über Datei_ weiter oben).

Folgende Parameter können mit angegeben werden:
