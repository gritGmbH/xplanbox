...
=== XPlanWMS - Darstellung von Plämen mit Gültigkeitszeitraum

Bei einer GetMap-Anfrage zum Zeitpunkt ausserhalb des für einen Plans angegebenen Gültigkeitszeitraums, werden derzeit keine Vektordaten des Plans angezeigt. Dies gilt für alle XPlanWMS unabhängig vom Planststatus. Die Rasterdaten werden dagegen wie für den XPlanWMSArchiviert und XPlanWMSFestgestellt angezeigt und für den XPlanWMSInAufstellung nicht.
...
=== XPlanManagerWeb und XPlanManagerAPI - Sortierung der Rasterdaten nach dem Editieren wird nicht angapsst

Wird durch das Editieren das für die Sortierung der Daten in der Visualisierung verwnendete Datumsfeld angepasst, erfolgt derzeit keine Anpassung der Sortierung der Rasterdaten, wenn diese durch deegree bereitgestellt werden. In Folge werden die Rasterdaten bei der Anzeige im XPlanMWS möglicherweise nicht wie erwartete Visualisiert. In diesem Fall muss der Adminstrator der xPlanBox das Werkzeug _xplan-update-data-cli/bin/sortDateUpdate_ ausführen um die Sortierung zu korrigieren. Hinweise dazu finden sich im Betriebshandbuch.
