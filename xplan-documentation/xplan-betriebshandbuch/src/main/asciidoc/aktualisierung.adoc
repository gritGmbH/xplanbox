...
| 2.12             | ja (<<aktualisierung-auf-die-version-2.12-der-xplanbox, Details>>) | ja (<<aktualisierung-auf-die-version-2.12-der-xplanbox, Details>>)
| 2.13             | ja (<<aktualisierung-auf-die-version-2.13-der-xplanbox, Details>>) | ja (<<aktualisierung-auf-die-version-2.13-der-xplanbox, Details>>)
...
Mit der Version 2.11 der xPlanBox sind verschiedene Erweiterungen umgesetzt worden. Für die Aktualisierung auf diese Version sind folgende Schritte auszuführen:

 * Aktualisierung der Workspaces und Konfigurationen (s. <<teilweise-aktualisierung>>)
 * Aktualisierung der Datenbank:
 ** Ausführen der Skripte im Verzeichnis _from_2.10_to_2.11_ im Modul xplan-update-database
...
[[aktualisierung-auf-die-version-2.12-der-xplanbox]]
=== Aktualisierung auf die Version 2.12 der xPlanBox

Mit der Version 2.12 der xPlanBox sind verschiedene Erweiterungen umgesetzt worden. Für die Aktualisierung auf diese Version sind folgende Schritte auszuführen:

 * Aktualisierung der Workspaces und Konfigurationen (s. <<teilweise-aktualisierung>>)
 * Aktualisierung der Datenbank:
 ** Ausführen der Skripte im Verzeichnis _from_2.11_to_2.12_ im Modul xplan-update-database

Mit der Version 2.12 wird bei entprechender Konfiguration (s. <<konfiguration-daten-dienste-kopplung>>) beim Import eines Plans geprüft ob für diesen Plan bereits ein Daten-Metadatensatz existiert. Ist dies der Fall wird automatisiert ein Service-Metadatensatz für den Plan bzw. dessen XPlanWerkWMS erstellt.
...
[[aktualisierung-auf-die-version-2.13-der-xplanbox]]
=== Aktualisierung auf die Version 2.13 der xPlanBox

In der Version 2.13 der xPlanBox wurden kleinere Anpassungen und Fehlerbehebungen vorgenommen. Für die Aktualisierung auf diese Version sind folgende Schritte auszuführen:

 * Aktualisierung der Workspaces und Konfigurationen (s. <<teilweise-aktualisierung>>)
 ** Umbenennung des Property mit dem Namen _directoryToStoreDatasetMetadata_ nach _directoryToStoreMetadata_ in der Datei _managerConfiguration.properties_ im Verzeichnis _~/.deegree/manager-configuration/_.
 * Aktualisierung der Datenbank:
 ** Ausführen der Skripte im Verzeichnis _from_2.12_to_2.13_ im Modul xplan-update-database
