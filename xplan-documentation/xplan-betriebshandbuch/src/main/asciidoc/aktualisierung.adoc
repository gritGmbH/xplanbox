...
* <<austausch-commandline-tool, Austausch des Kommandozeilenwerkzeugs>>
...
[[austausch-commandline-tool]]
=== Austausch des Kommandozeilenwerkzeugs

Das ZIP-Archiv mit dem <<kommandozeilen-anwendung,Kommandozeilenwerkzeug>> muss entpackt werden:

----
unzip ${WORK_DIR}/bin/xplan-cli.zip
----
...
| 8.0   (<<aktualisierung-version-8.0, Details>>)       | ja   | ja
...
Zur Aktualisierung der Daten steht das <<kommandozeilen-anwendung, Kommandozeilenwerkzeug XPlanCLI>> zur Verfügung. Nach einer Aktualisierung der xPlanBox muss folgendes Kommando ausgeführt werden:

. _xpb admin resynthesize_

Weitere Informationen zum Kommandozeilenwerkzeug sind im Abschnitt <<kommandozeilen-anwendung>> beschrieben.
...
* Ausführen des <<kommandozeilen-anwendung, Kommandozeilenwerkzeugs XPlanCLI>> mit dem Kommando _xpb admin resynthesize_ zur Aktualisierung der in der XPlanSyn-Datenhaltung gespeicherten Daten.
...
[[aktualisierung-version-8.0]]
=== Aktualisierung auf die Version 8.0 der xPlanBox

Für die Aktualisierung auf die Version 8.0 sind folgende Schritte auszuführen:

* Aktualisierung der Workspaces und Konfigurationen (s. <<teilweise-aktualisierung>>)
* Aktualisierung der Datenbank:
** Ausführen der Liquibase-Skripte, wie im Abschnitt <<konfiguration-xplandb-liquibase, Liquibase>> beschrieben

NOTE: Die Namen der Layer im XPlanWMS, XPlanArtWM, XPlanWerkWMS, XPlanValidatorWMS, MapServer und MapProxy haben sich geändert. Im https://gitlab.opencode.de/diplanung/ozgxplanung/-/blob/main/CHANGELOG.md[CHANGELOG.md] sind die Änderungen aufgelistet. Die Konfigurationen der GIS Clients, die die WMS-Dienste einbinden, müssen ggf. angepasst werden. Weiterhin müssen aufgrund der Änderungen alle Service-Metadatensätze neu generiert und in den verwendeten CSW importiert werden (s. dazu den Abschnitt zur <<konfiguration-daten-dienste-kopplung, automatisierten Erstellung von Service-Metadatensätzen>>.