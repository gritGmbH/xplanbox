...
NOTE: Bei der Aktualisierung kann es zu folgender Fehlermeldungen kommen: _ERROR:  relation "databasechangeloglock" already exists_ kommen. Diese Fehlermeldung kann ignoriert werden.
...
IMPORTANT: Die Skripte für die Aktualisierung auf die Version 5.0 werden mit der Version 6.0 der xPlanBox nicht mehr ausgeliefert. Bei  Fragen zur Aktualisierung von früheren Versionen (5.0 und älter), wenden Sie sich bitte an die https://www.lat-lon.de[lat/lon GmbH].
