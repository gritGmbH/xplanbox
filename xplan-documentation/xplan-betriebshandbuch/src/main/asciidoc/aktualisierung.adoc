...
CAUTION: Im Verzeichnis _themes/_ nicht die Dateien, die auf _raster.xml_ enden, ersetzen!
...
In der Version 6.1 der xPlanBox erfolgte eine Umstellung des Loggings. Es werden für die Anwendungen XPlanManagerWeb, XPlanManagerApi, XPlanValidatorWeb und XPlanValidatorApi, sowie alle XPlanDienste keine Log-Dateien mehr geschrieben. Die Log-Ausgaben werden standardmäßig nur noch nach Standard-Out geschrieben.
