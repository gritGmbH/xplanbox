...
NOTE: Diese Konfiguration ist für alle drei Varianten erforderlich! Wenn der Servlet-Filter aktiviert ist, muss zusätzlich noch eine drei Varianten konfiguriert werden. Wie die Absicherung deaktiviert werden kann ist in <<disable_security>> beschrieben.
...
Die Vorgabewerte für die beiden vorkonfigurierten Benutzer "xplanisk" (Rolle _SUPERUSER_) und "guest" (Rolle _USER_) sind in der Konfgurationsdatei _WEB-INF/classes/security.properties_ gesetzt.
...
Die Konfiguration eines UserService ist in der Referenzdokumentation von Spring Security beschrieben:
...
Weitere Informationen zur LDAP/AD-Anbindung in der Referenzdokumentation von Spring Security:
...
werden, damit die Änderungen von der Software erkannt und genutzt werden können.
