...
NOTE: Die nicht abgebildete Tabelle planslog beinhaltet eine Historie jedes Plans ab dem Zeitpunkt an dem die Tabelle inkl. Function und Trigger angelegt wurde. Die Historie umfasst den Zeitpunkt wann ein Plan importiert (INSERT), aktualisiert (UPDATE) und gelöscht (DELETE) wurde. Außerdem wird die Version, die Planart, die BoundingBox des Plans sowie der neue und der alte Planstatus abgespeichert.