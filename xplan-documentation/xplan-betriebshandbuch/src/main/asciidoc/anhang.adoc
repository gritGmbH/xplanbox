...
Als Basisverzeichnis wird _<DEEGREE_WORKSPACE_ROOT>_ und temporär zur Ablage das Verzeichnis _<BACKUP_CONFIG>_ (z.B. _/home/user/tmp_) vorausgesetzt.

Vorbereitung:

1. Alle deegree Workspace Verzeichnisse aus _<DEEGREE_WORKSPACE_ROOT>_ sichern und im Verzeichnis _<BACKUP_CONFIG> ablegen.
...
Konfiguration im Verzeichnis _<DEEGREE_WORKSPACE_ROOT>/xplan-manager-workspace_ aktualisieren:

* _<BACKUP_CONFIG>/xplan-manager-workspace/jdbc_ kopieren

Konfiguration im Verzeichnis _<DEEGREE_WORKSPACE_ROOT>/xplansyn-wfs-workspace_ aktualisieren:

* _<BACKUP_CONFIG>/xplansyn-wfs-workspace/jdbc_ kopieren
* _<BACKUP_CONFIG>/xplansyn-wfs-workspace/services/xplansynwfs.xml_ kopieren

Konfiguration im Verzeichnis _<DEEGREE_WORKSPACE_ROOT>/xplansyn-wms-workspace_ aktualisieren:

* _<BACKUP_CONFIG>/xplansyn-wms-workspace\jdbc_ kopieren
...
* _<BACKUP_CONFIG>/xplansyn-wms-workspace\data_ zurückkopieren
* _<BACKUP_CONFIG>/xplansyn-wms-workspace\datasources\tile_ zurückkopieren
* _<BACKUP_CONFIG>/xplansyn-wms-workspace\layers_ alle Layer, die mit einer Nummer anfangen, zurückkopieren
* _<BACKUP_CONFIG>/xplansyn-wms-workspace\themes\*raster.xml_ (alle Datei die auf _raster.xml_ enden) zurückkopieren

Konfiguration im Verzeichnis _<DEEGREE_WORKSPACE_ROOT>/xplan-wfs-workspace_ aktualisieren:

* _<BACKUP_CONFIG>/xplan-wfs-workspace\jdbc_ kopieren

Konfiguration im Verzeichnis _<XPLANBOX_CONFIG>/xplan-inspireplu-workspace_ aktualisieren:

* _<BACKUP_CONFIG>/xplan-inspireplu-workspace\jdbc_ kopieren
...
Der Workspace _xplansyn-wms-workspace_ wird durch den XPlanManager verwaltet. Werden Planarchive mit Rasterdaten über den XPlanManager importiert, dann werden
...
<2> Die Datei _gdal.ignore_ ist nur erforderlich, wenn kein <<konfiguration-gdal, GDAL>> verwendet wird. Wird GDAL verwendet, dann muss die Konfigurationsdatei _gdal.xml_ angelegt werden.
<3> Im Verzeichnis _jdbc/_ liegt die Konfigurationsdatei mit der Datenbankverbindung zur <<XPlanDB>>.
<4> Im Verzeichnis _layers/_ werden die Rasterdaten (PNG oder GeoTIFF) abgelegt. Zu jeder Rasterdatei wird durch den XPlanManager eine Konfigurationsdatei erzeugt.
<5> Im Verzeichnis _legends/_ liegen die Legendensymbole.
<6> Im Verzeichnis _services/_ liegen die Konfigurationsdateien für den <<XPlanWMS>>.
<7> Im Verzeichnis _styles/_ liegen die Zeichenvorschriften für den XPlanWMS.
<8> Im Verzeichnis _symbols/_ liegen die Symbole die in den Zeichenvorschriften verwendet werden.
<9> Im Verzeichnis _themes/_ liegen die Konfigurationsdateien für den XPlanWMS, die den Ebenenbaum definieren. Die generierten Konfigurationsdateien für die einzelnen Ebenen werden beim Import vom XPlanManager erweitert.
...
Die Tabellenstruktur unterscheidet sich nicht zu den anderen von der xPlanBox unterstützten XPlanGML-Versionen. Nur die Inhalte in der Tabelle `feature_types` unterscheiden sich je nach XPlanGML-Version.

=== Datenbankschema für XPlanSyn

Das XPlanSyn-Schema wird aus den von der xPlanBox unterstützten XPlanGML-Versionen generiert.
In der aktuellen Version der xPlanBox sind dies die XPlanGML-Versionen 3.0 bis 5.3. Instanzdokumente werden im XPlanSyn-Schema unter Verwendung des relationalen Mappings von deegree (siehe https://download.deegree.org/documentation/current/html/#_mapping_gml_application_schemas[deegree webservices relational mapping]) gespeichert.
...
Eine vollständige Übersicht der 324 Tabellen des Datenbankschemas `xplansyn` ist in der Datei _xplandb_xplansyn_komplett.png_ enthalten.
