...
XPlanWMS und XPlanWerkWMS aktualisieren soll, muss ein Workspace-Reload ausgeführt werden. Damit der XPlanManager auf die REST-Schnittstelle des XPlanWMS/XPlanWerkWMS zugreifen kann, muss die Authentifzierung über ein API Key eingerichtet werden. Ansonsten ist ein manueller Workspace-Reload nötig, um die neu
...
Der API Key wird beim Start von deegree automatisch angelegt und ein generierter Key gesetzt. Dieser kann nachträglich angepast werden. Weitere Informationen finden sich um Handbuch von https://download.deegree.org/documentation/current/html/#%5Fsetting%5Fup%5Fthe%5Finterface[deegree webservices].
