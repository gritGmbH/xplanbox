...
** *id*: Ein eindeutiger Bezeichner für das Profil, wenn dieser nicht angegeben wird, dann wird dieser aus `name` und `description` zusammengesetzt. (optional)
...
** *xqueryRulesDirectory*: Die absolute Pfadangabe auf das Verzeichnis in dem die XQuery-Regeln abgelegt sind. (verpflichtend)
...
** Im Verzeichnis muss eine Datei mit dem Namen _rules.properties_ liegen.
...
.Beispiel für die Verzeichnisstruktur eines Profils:
----
nrw
├── rules.properties        # <1>
