...
* Die YAML-Datei beinhaltet die Konfiguration für ein Profil, das durch folgende Eigenschaften beschrieben wird:
** *name*: Der Name des Profils, dieser wird im XPlanValidatorWeb und XPlanManagerWeb angezeigt (Angabe ist verpflichtend)
...
- name: Pflichtenheft NRW
  ...
  xqueryRulesDirectory: /root/xplan-validator-config/profiles/nrw <1>
  rulesDescription: /root/xplan-validator-config/profiles/nrw/rules.properties <2>
----
