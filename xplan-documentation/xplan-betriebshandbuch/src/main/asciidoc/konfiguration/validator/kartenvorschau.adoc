...
Die Aktivierung des jeweiligen Workspace erfolgt über die Datei _xplan-validator-workspaces/webapps.properties_.

.Beispiel für webapps.properties mit der Auswahl der XPlanDB als Datenhaltung für den XPlanValidatorWMS:
[source,properties]
----
/xplan-validator-wms=xplan-validator-wms-sql-workspace
----

NOTE: Für die Aktivierung der XPlanDB als Datenhaltung muss zusätzlich auch die Umgebungsvariable `spring_profiles_active=validatorwmssql` gesetzt werden.