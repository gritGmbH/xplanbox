...
Der Zeitraum in dem validierte Pläne vorgehalten werden kann bei Bedarf angepasst werden. Die Konfiguration unterscheidet sich je nachdem ob die Daten im Arbeitsspeicher oder in der XPlanDB abgelegt werden:

* Für die Speicherung der Daten im Arbeitsspeicher: Setzen der Umgebungsvariable `DELETE_AFTER_MINUTES=15` für den XPlanValidatorWMS.
* Für die Speicherung der Daten in der XPlanDB: Setzen der Umgebungsvariable `DELETE_AFTER_MINUTES=15` für XPlanValidatorWeb und XPlanValidatorAPI.

Im Beispiel wird der Zeitraum in der die Daten über den XPlanValidatorWMS sichtbar sind auf 15 Minuten erhöht.