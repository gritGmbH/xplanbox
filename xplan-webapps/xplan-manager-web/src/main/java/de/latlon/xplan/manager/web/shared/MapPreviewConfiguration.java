...
public class MapPreviewConfiguration implements Serializable {
    ...
    /**
     * @param basemapUrl
     *            the basemap wms url, never <code>null</code> must be of the following template format:
     *            http://{host}/{service
     *            }?SERVICE=WMS&REQUEST=GetMap&FORMAT={format}&BBOX={bbox}&SRS={srs}&VERSION={version
     *            }&layers={layername}
     * @param basemapName
     *            the name of the basemap, may be <code>null</code>
     * @param basemapLayer
     *            the layer of the basemap, never <code>null</code>
     * @param wmsUrl
     *            the xplan wms url, never <code>null</code> must be of the following template format:
     *            http://{host}/{service}. If the url ends with '?' or not with 'services', the configured url is used
     *            as wms url, otherwise the wmsEndpoint or wmsPreEndpoint is appended (if configured).
     * @param wmsEndpoint
     *            endpoint of the default wms, providing the plans with status 'FESTGESTELLT', may be <code>null</code>
     *            (if the service only provides one wms or the wmsUrl is configured as complete endpoint)
     * @param wmsPreEndpoint
     *            endpoint of the default wms, providing the plans with status 'IN_AUFSTELLUNG', may be
     *            <code>null</code> (if the service only provides one wms or the wmsUrl is configured as complete
     *            endpoint)
     * @param wmsArchiveEndpoint
     *            endpoint of the default wms, providing the plans with status 'ARCHIVIERT', may be <code>null</code>
     *            (if the service only provides one wms or the wmsUrl is configured as complete endpoint)
     * @param mapExtent
     *            the max extent of the map (with crs), never <code>null</code>
     * @param vectorLayerConfiguration
     *            the vector layer configuration, never <code>null</code>
     * @param rasterLayerConfiguration
     *            the raster layer configuration, never <code>null</code>
     */
    public MapPreviewConfiguration( String basemapUrl, String basemapName, String basemapLayer, String wmsUrl,
                                    String wmsEndpoint, String wmsPreEndpoint, String wmsArchiveEndpoint,
