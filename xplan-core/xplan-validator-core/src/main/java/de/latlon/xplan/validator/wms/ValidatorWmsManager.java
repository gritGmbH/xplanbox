...
public class ValidatorWmsManager {
	 ...
	 * @param workspaceLocation path to the workspace
	 * xplan-validator-wms-memory-workspace, the directory data is created if required
	 * @throws IOException if the directory data could not be required
	 * @throws IllegalArgumentException if the workspace location or file
	 * datasources/feature/xplansyn.xml does not exixt
	 */
	public ValidatorWmsManager(XPlanSynthesizer synthesizer, Path workspaceLocation) throws IOException {
	...
	/**
	 * @param featureCollection feature collection to append to the
	 * {@link MemoryFeatureStoreConfig}, never <code>null</code>
	 * @throws MapPreviewCreationException if the configuration could not be writtem
	 * @return
	 */
