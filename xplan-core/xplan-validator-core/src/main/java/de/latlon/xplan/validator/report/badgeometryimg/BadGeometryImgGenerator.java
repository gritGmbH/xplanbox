...
public class BadGeometryImgGenerator {
	 ...
	 * @param report to check for bad geometries, never <code>null</code>
	 * @return <code>true</code> if the report contains {@link BadGeometry}s,
	 ...
	 * @param report to retrieve bad geometries from, never <code>null</code>
	 * @param os the OutputStream where the Image is written into, never <code>null</code>
	 * @throws ReportGenerationException if an exception occurred during writing the image
	 */
	public void generateReport(ValidatorReport report, OutputStream os) throws ReportGenerationException {
	...
	private void writePNGImage(OutputStream os, List<BadGeometry> badGeometries) throws ReportGenerationException {
	...
	private void renderBadGeometries(List<BadGeometry> badGeometries, PointStyling defaultPointStyle,
			LineStyling defaultLineStyle, PolygonStyling defaultPolygonStyle, RenderContext ctx) {
		...
		for (BadGeometry badGeometry : badGeometries) {
			...
			badGeometry.getMarkerGeometries().values().forEach(
					g -> renderGeometry(defaultPointStyle, defaultLineStyle, defaultPolygonStyle, renderer, g));
		}
	}
	...
	private void renderGeometry(PointStyling defaultPointStyle, LineStyling defaultLineStyle,
		...
		try {
			if (!Geometry.GeometryType.PRIMITIVE_GEOMETRY.equals(geom.getGeometryType())) {
				LOG.warn(
						"Geometry is not primitive, rendering of non primitive geometries in png is currently not supported ");
				return;
			}
	...
	private void writeToOutputStream(OutputStream os, ByteArrayOutputStream baos) throws IOException {
	...
	private Envelope calculateBoundingBoxOfBadGeometries(List<BadGeometry> badGeometries) {
		// the envelope has to be bigger than the geometry, otherwise an exception will
		// occur
