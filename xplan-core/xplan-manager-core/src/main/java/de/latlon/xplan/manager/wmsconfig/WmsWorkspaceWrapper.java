...
public class WmsWorkspaceWrapper {
     ...
     * 
     * @param configuredCrs
     *            the configured crs of the themes configuration, may be <code>null</code> (check of the themes crs is
     *            skipped)
     * @throws JAXBException
     * @throws IOException
     * @throws ConfigurationException
     */
    public void checkThemes( String configuredCrs )
    ...
    /**
     * @param type
     *            the type of the configuration to retrieve, never <code>null</code>
    ...
    private void checkTheme( String type, File config, Themes themes )
        ...
        if ( !expected.equals( firstChildIdentifier ) ) {
            throw new ConfigurationException( "Datei '" + config + "' hat eine nicht unterstützte Struktur. "
                                              + " Das erste Sub-Theme hat den Identifier: " + firstChildIdentifier
                                              + ". Erwartet: " + expected );
        }
    }

    private Themes parseThemes( File config )
        ...
        try ( InputStream configFileStream = new FileInputStream( config ) ) {
            return (Themes) JAXBUtils.unmarshall( "org.deegree.theme.persistence.standard.jaxb",
                                                  new StandardThemeProvider().getSchema(), configFileStream,
                                                  workspace.getNewWorkspace() );
        }
    }

    private void checkThemeCrs( File config, Themes themes, String configuredCrs )
                    throws ConfigurationException {
        if ( configuredCrs == null )
            return;
        ThemeType theme = themes.getTheme();
        if ( !configuredCrs.equalsIgnoreCase( theme.getCRS() ) ) {
            throw new ConfigurationException( "In der Themes Konfiguration '" + config
                                              + "' ist ein anderes CRS konfiguriert (" + theme.getCRS()
