...
class AppSchemaMapper {
     ...
     * @param createRelationalMapping true, if relational mapping should be performed, false otherwise
     * @param geometryParams          parameters for storing geometries, must not be <code>null</code>
     */
    ...
    private List<Mapping> generatePropMapping( PropertyType pt, MappingContext mc ) {
        ...
        try {
            ...
            LOG.warn(
                  "Unable to create relational mapping for property type '" + pt.getName() + "': " + t.getMessage() );
        }
    ...
    private List<Mapping> generateMapping( XSComplexTypeDefinition typeDef, MappingContext mc,
        ...
        if ( typeDef.getName() != null ) {
            for ( XSComplexTypeDefinition ct : parentCTs ) {
                if ( ct.getName() != null ) {
                    if ( typeDef.getName().equals( ct.getName() ) && typeDef.getNamespace()
                          .equals( ct.getNamespace() ) ) {
