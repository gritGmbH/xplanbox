 ...
 */
public class SynchronizeLgvSynSchemaTool {

    private static final Logger LOG = LogManager.getLogger( SynchronizeLgvSynSchemaTool.class );

    private enum SYNCTYPE {ALL, SYNC}
    ...
    public static void main( String[] args ) {
        if ( args == null || args.length == 0 || ( args.length > 0 && ( args[0].contains( "help" ) || args[0].contains(
                        "?" ) ) ) ) {
        ...
        try {
            ...
            try {
                ...
                SynchronizeLgvSynSchemaTool tool = new SynchronizeLgvSynSchemaTool();
    ...
    private void run( SYNCTYPE synctype, String host, String port, String database, String user, String password ) {
        ...
        case SYNC:
            sync( host, port, database, user, password, ( conn ) -> {
                LgvSynSchemaSynchronizer synchronizer = new LgvSynSchemaSynchronizer();
    ...
    private static void printHelp( Options options ) {
        ...
        String toolName = SynchronizeLgvSynSchemaTool.class.getSimpleName();
