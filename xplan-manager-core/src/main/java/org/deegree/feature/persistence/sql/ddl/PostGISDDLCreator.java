...
// trunk/deegree-datastores/deegree-featurestore/deegree-featurestore-sql/
// src/main/java/org/deegree/sqldialect/postgis/PostGISDDLCreator.java $
/*----------------------------------------------------------------------------
...
class PostGISDDLCreator extends DDLCreator {
    ...
    protected void featureMappingSnippet( StringBuffer sql, FeatureMapping mapping ) {
        SQLIdentifier col = mapping.getJoinedTable().get( mapping.getJoinedTable().size() - 1 ).getFromColumns()
              .get( 0 );
        if ( col != null ) {
            sql.append( ",\n    " );
            sql.append( col );
