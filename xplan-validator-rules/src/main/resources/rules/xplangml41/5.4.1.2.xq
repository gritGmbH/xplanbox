...
(
  ...
  (
    ...
    then (
      ...
      not(exists($h/weitereBesondZweckbestimmung4)) and
      not(exists($h/weitereBesondZweckbestimmung5))
    )
    else boolean('false')
  )
)
and
(
  every $h in //FP_Gemeinbedarf[besondereZweckbestimmung[2]] satisfies
  not(exists($h/weitereBesondZweckbestimmung1)) and
  not(exists($h/weitereBesondZweckbestimmung2)) and
  not(exists($h/weitereBesondZweckbestimmung3)) and
  not(exists($h/weitereBesondZweckbestimmung4)) and
...
(
  ...
  (
    if (
      ...
      exists($h/weitereBesondZweckbestimmung4) and
      exists($h/weitereBesondZweckbestimmung5)
    )
    then (
      boolean('true')
    )
