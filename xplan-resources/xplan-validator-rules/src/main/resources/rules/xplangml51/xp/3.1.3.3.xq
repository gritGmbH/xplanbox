...
(
  every $planinhalt in //BP_Bereich/planinhalt satisfies
  (
    let $objectId := substring($planinhalt/@xlink:href, 2)
    let $bereichRef := concat('#', $planinhalt/../@gml:id)
    return local:checkRef($xplanauszug, $objectId, $bereichRef)
  )
) and (
  every $planinhalt in //FP_Bereich/planinhalt satisfies
  (
    let $objectId := substring($planinhalt/@xlink:href, 2)
    let $bereichRef := concat('#', $planinhalt/../@gml:id)
    return local:checkRef($xplanauszug, $objectId, $bereichRef)
  ...
  every $planinhalt in //LP_Bereich/planinhalt satisfies
  (
    let $objectId := substring($planinhalt/@xlink:href, 2)
    let $bereichRef := concat('#', $planinhalt/../@gml:id)
    return local:checkRef($xplanauszug, $objectId, $bereichRef)
  ...
  every $planinhalt in //RP_Bereich/planinhalt satisfies
  (
    let $objectId := substring($planinhalt/@xlink:href, 2)
    let $bereichRef := concat('#', $planinhalt/../@gml:id)
    return local:checkRef($xplanauszug, $objectId, $bereichRef)
  ...
  every $planinhalt in //SO_Bereich/planinhalt satisfies
  (
    let $objectId := substring($planinhalt/@xlink:href, 2)
    let $bereichRef := concat('#', $planinhalt/../@gml:id)
    return local:checkRef($xplanauszug, $objectId, $bereichRef)
