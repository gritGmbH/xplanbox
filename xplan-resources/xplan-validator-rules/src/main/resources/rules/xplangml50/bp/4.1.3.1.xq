...
(
every $ausgeglichendurch in //wirdAusgeglichenDurchFlaeche satisfies
let $id := substring($ausgeglichendurch/@xlink:href, 2)
let $featuretypename := local-name($ausgeglichendurch/..)
return
index-of($featuretypes, $featuretypename) and exists(//BP_AusgleichsFlaeche[@gml:id = substring($id, 2)])
)